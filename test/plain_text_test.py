...
def gui_load(gui_init):
    """Start GUI"""
    QTimer.singleShot(150, lambda: open_config_file("./test/pttest.txt"))
    gui_init.load_file()
    return gui_init
...
def run():
    ...
    return StackedWindowGui("./test/pttest.txt")
...
def test_create(gui_init, qtbot):
    ...
    for row in range(layout.rowCount()):
        if type(layout.itemAt(row, QFormLayout.ItemRole.FieldRole)) == QWidgetItem:
            not_none_rows += 1
            assert layout.itemAt(row, QFormLayout.ItemRole.LabelRole).widget().text() in fields_per_type["Plain Text"][0].keys()
